...
class TestSpectrometer(baseclass):
    ...
    def __init__(self, spectrometer: Spectrometer, dye: Dye, parent=None):
        ...
        self.plot_item = self.pw.plotItem.plot(name="Measured", pen=pyqtgraph.mkPen("w",width=1.5))
        self.pw.plotItem.setLabel(
            axis="bottom", text="Wavelength (nm)"
        )
        ...
        self.pw.plotItem.getViewBox().setLimits(minXRange=self.x[0],maxXRange=self.x[-1],minYRange=0,maxYRange=65535,xMin=self.x[0],xMax=self.x[-1],yMin=0)
        
        # Init cursors
        self.pix_ex = np.abs(self.x - self.dye.wavelength_excitation).argmin()
        self.boxcar_pix_ex = [
            np.abs(self.x - (self.dye.wavelength_excitation-(self.dye.boxcar_width/2))).argmin(),
            np.abs(self.x - (self.dye.wavelength_excitation+(self.dye.boxcar_width/2))).argmin()
        ]
        self.tw.item(0,0).setText(str(self.dye.wavelength_excitation))
        self.tw.item(1,0).setText(f"{self.x[self.pix_ex]:.1f}")
        self.tw.item(3,0).setText(f"[{self.x[self.boxcar_pix_ex[0]]:.1f},\n{self.x[self.boxcar_pix_ex[-1]]:.1f}]")
        self.tw.item(4,0).setText(str(self.boxcar_pix_ex[-1]-self.boxcar_pix_ex[0]+1))
        ex_rgb = self._get_rgb_from_wavelength(self.dye.wavelength_excitation)
        self.pw.addItem(pyqtgraph.InfiniteLine(pos=self.x[self.pix_ex],
                               pen=pyqtgraph.mkPen(QColor(ex_rgb[0],ex_rgb[1],ex_rgb[2])),
                               label="Excitation = {value:.1f}",
                               labelOpts={"color": QColor(ex_rgb[0],ex_rgb[1],ex_rgb[2]), "position": 0.9,"anchors": [(1,0.9),(1,0.9)],}))
        
        self.pix_em = np.abs(self.x - self.dye.wavelength_emission).argmin()
        self.boxcar_pix_em = [
            np.abs(self.x - (self.dye.wavelength_emission-(self.dye.boxcar_width/2))).argmin(),
            np.abs(self.x - (self.dye.wavelength_emission+(self.dye.boxcar_width/2))).argmin()
        ]
        self.tw.item(0,1).setText(str(self.dye.wavelength_emission))
        self.tw.item(1,1).setText(f"{self.x[self.pix_em]:.1f}")
        self.tw.item(3,1).setText(f"[{self.x[self.boxcar_pix_em[0]]:.1f},\n{self.x[self.boxcar_pix_em[-1]]:.1f}]")
        self.tw.item(4,1).setText(str(self.boxcar_pix_em[-1]-self.boxcar_pix_em[0]+1))
        em_rgb = self._get_rgb_from_wavelength(self.dye.wavelength_emission)
        
        self.pw.addItem(pyqtgraph.InfiniteLine(pos=self.x[self.pix_em],
                               pen=pyqtgraph.mkPen(QColor(em_rgb[0],em_rgb[1],em_rgb[2])),
                               label="Emission = {value:.1f}",
                               labelOpts={"color": QColor(em_rgb[0],em_rgb[1],em_rgb[2]), "position": 0.9,"anchors": [(0,0.9),(0,0.9)],}))
        ...
        self.label_serial.setText(f"Spectrometer Serial Number: {self.spec.serial_number}")
        self.label_integration_Time.setText(f"Target Integration Time: {self.dye.integration_time_milliseconds} milliseconds")
