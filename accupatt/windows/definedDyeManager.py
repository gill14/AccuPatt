...
from PyQt6.QtCore import QAbstractListModel, QModelIndex, Qt, pyqtSlot, QItemSelectionModel
...
class DyeManager(baseclass):
    ...
    def __init__(self, parent=None):
        ...
        self.lv.selectionModel().currentChanged[QModelIndex,QModelIndex].connect(self.current_dye_changed)
    ...
    def excitation_changed(self):
        self.lm.dyes[self.lv.currentIndex().row()].wavelength_excitation = int(self.le_ex.text())
        
    @pyqtSlot()
    def emission_changed(self):
        self.lm.dyes[self.lv.currentIndex().row()].wavelength_emission = int(self.le_em.text())
        
    @pyqtSlot()
    def integration_time_changed(self):
        self.lm.dyes[self.lv.currentIndex().row()].integration_time_milliseconds = int(self.le_it.text())
